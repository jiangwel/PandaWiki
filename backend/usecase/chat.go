...
func (u *ChatUsecase) ChatRagOnly(ctx context.Context, req *domain.ChatRequest) (<-chan domain.SSEEvent, error) {
	...
	go func() {
		...
		if err != nil {
			u.logger.Error("failed to marshal messages", log.Error(err))
			eventCh <- domain.SSEEvent{Type: "error", Content: "failed to marshal messages"}
			return
		}
		eventCh <- domain.SSEEvent{Type: "message", Content: string(b)}
